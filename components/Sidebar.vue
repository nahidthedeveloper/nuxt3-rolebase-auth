...
const sideList = [
    { "id": 1, "name": "View Todos", "path": "/todos/list", "permission": "todos.view_todos" },
    { "id": 2, "name": "Add Todos", "path": "/todos/add", "permission": "todos.add_todos" },
    { "id": 3, "name": "Edit Todos", "path": "/todos/edit", "permission": "todos.change_todos" },
    { "id": 4, "name": "Delete Todos", "path": "/todos/delete", "permission": "todos.delete_todos" }
];

const filteredSideList = computed(() => {
    const permissions = userPermissionsStore.userPermissionsList || [];
    return sideList.filter(item =>
        permissions.some(per => per.name === item.permission)
    );
});
</script>
    ...
    <div :class="{ 'translate-x-0': usersStore.isOpen, '-translate-x-full': !usersStore.isOpen }"
        ...
        <div class="py-4 overflow-y-auto">
            <ul class="space-y-2 font-medium">
                <li v-for="list in filteredSideList" :key="list.id">
